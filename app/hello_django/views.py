    ...
    if response.status_code == 200:
        ...
        if user_response.status_code == 200:
            ...
            if created:
                ...
                #user.save()
                logger.critical("Informations de l'utilisateur enregistrées avec succès : %s", user.first_name)
                logger.critical("Informations de l'utilisateur enregistrées avec succès : %s", user.username)
                u = usertest(login=user.username, first_name=user.first_name, last_name=user.last_name, image=user_data.get('image', ''))
                u.save()
            else:
                h = usertest(login=user.username, first_name=user.first_name, last_name=user.last_name, image=user_data.get('image', ''))
                logger.critical("login : %s", h.login)
                logger.critical("prenom : %s", h.first_name)
                logger.critical("nom : %s", h.last_name)
                logger.critical("image : %s", h.image)
                h.save()
        else:
